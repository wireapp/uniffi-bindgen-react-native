...
macro_rules! export_jspi_entry {
    () => {
        ...
        // Invoking this macro explicitly opts into wasm-bindgen's experimental JSPI API.
        #[expect(deprecated)]
        mod __ubrn_jspi_entry {
            ...
            pub unsafe fn __ubrn_jspi_enter(target: u32, frame: u32) {
                let call: unsafe extern "C" fn(u32) =
                    unsafe { ::std::mem::transmute(target as usize) };
                unsafe { call(frame) };
            }
        }
        #[cfg(target_arch = "wasm32")]
        pub use __ubrn_jspi_entry::__ubrn_jspi_enter;
    };
}
