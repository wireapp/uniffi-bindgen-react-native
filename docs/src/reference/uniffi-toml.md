...
```toml
[bindings.typescript]
jspi = ["fetchData", "Client.refresh", "Client.open"]
```

Selectors support:

- `fetchData`: one top-level function.
- `Client`: all constructors, methods, and exported trait helpers
  (`Display`, `Debug`, `Eq`, `Hash`, `Ord`) on an object, record, or enum type.
- `Client.refresh`: one method or named Rust constructor (`Client.open`).
- `Client.constructor`: the primary Rust constructor, usually named `new`.
  Use this selector even though a selected primary constructor is exposed as
  the TypeScript static `create` factory; `Client.new` is not an alias.

Each side of the dot uses the same case normalization as `forceAsync`:
`client_type.fetchData` matches `ClientType.fetch_data`. Qualified selectors
address exported Rust members, not generated JavaScript helpers or lifecycle
methods. Trait helpers are selected through their whole type.
Unknown, malformed, or ambiguous selectors (including member names that collide
after normalization) produce errors. Repeated or overlapping selectors are
combined: `["Client", "Client.refresh"]` has the same effect as `["Client"]`.
`jspi = true` selects all eligible functions and types; `false` (the default) or
an empty list disables it. Existing `forceAsync` selection is unchanged.

To select every eligible callable declared `async fn` in Rust without changing
the TypeScript surface of synchronous callables, use the automatic form:
...
Synchronous Rust entry points can also reach suspending JavaScript imports; this
cannot be inferred from UniFFI metadata. Add any such entry points explicitly:

```toml
[bindings.typescript]
jspi = { rustAsync = true, include = ["Client.synchronousButSuspending"] }
```

`include` accepts the same function, type, and `Type.member` selectors described
above. Automatic selection skips callback-capable and backend-ineligible
callables; explicitly included unsupported callables remain configuration errors.

Use `exclude` to subtract callables from a selection. When `exclude` is the only
option, the starting selection is every eligible callable, so selecting everything
except a primary constructor is concise:

```toml
[bindings.typescript]
jspi = { exclude = ["Client.constructor"] }
```

With `rustAsync` or `include`, exclusions are instead subtracted from that
narrower selection. `exclude` accepts the same selectors as `include`; unknown,
ambiguous, or unsupported selectors are configuration errors.

Only selected callables gain JSPI. To preserve an existing API, select only its
Rust async methods and constructors. For example, the configuration above keeps
`new Client(...)`, unselected named constructors, and sync methods synchronous,
including the primary constructor's visibility and subclassability. Selecting a
sync callable explicitly still changes its API to return a Promise.
...
types, including Rust async callables. Use the same `jspi` boolean, name-list, or
automatic configuration and case normalization described above. `true` selects
eligible functions and types. Selecting a type includes its Rust constructors,
methods and exported trait helpers; callback-capable types are excluded and
rejected when named. Unsupported callback/reference ABI shapes are also
excluded.
