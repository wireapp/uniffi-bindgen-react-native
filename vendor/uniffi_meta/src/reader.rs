...
struct MetadataReader<'a> {
    ...
    buf: &'a [u8],
    // UniFFI contract <=29 encoded default values directly as literals.
    legacy_default_encoding: bool,
}
...
impl<'a> MetadataReader<'a> {
    fn new(data: &'a [u8]) -> Self {
        Self {
            initial_data: data,
            buf: data,
            legacy_default_encoding: forced_contract_version()
    ...
    fn read_default(&mut self, name: &str, ty: &Type) -> Result<DefaultValueMetadata> {
        ...
        match default_kind {
            // UniFFI >=30 encoding.
            codes::DEFVALUE_DEFAULT => {
                if self.legacy_default_encoding {
                    Ok(DefaultValueMetadata::Literal(
                        self.read_literal_with_kind(name, ty, codes::LIT_STR)?,
                    ))
                } else {
                    Ok(DefaultValueMetadata::Default)
                }
            }
            // Ambiguous between:
            //   - UniFFI >=30: DEFVALUE_LITERAL + LIT_*
            //   - UniFFI <=29: LIT_INT
            codes::DEFVALUE_LITERAL => {
                let checkpoint = self.buf;
                match self.read_literal(name, ty) {
                    Ok(literal) => Ok(DefaultValueMetadata::Literal(literal)),
                    Err(new_encoding_err) => {
                        self.buf = checkpoint;
                        self.read_legacy_default(name, ty, default_kind)
                            .with_context(|| {
                                format!(
                                    "failed to decode default value using contract>=30 encoding: {new_encoding_err}"
                                )
                            })
                    }
                }
            }
            // UniFFI <=29 encoding where the default value kind is the literal kind itself.
            codes::LIT_FLOAT
            | codes::LIT_BOOL
            | codes::LIT_NONE
            | codes::LIT_SOME
            | codes::LIT_EMPTY_SEQ
            | codes::LIT_EMPTY_MAP => self.read_legacy_default(name, ty, default_kind),
            _ => bail!("Unexpected default value kind code: {default_kind:?}"),
        }
    }

    fn read_legacy_default(
        &mut self,
        name: &str,
        ty: &Type,
        literal_kind: u8,
        ...
        Ok(DefaultValueMetadata::Literal(
            self.read_literal_with_kind(name, ty, literal_kind)?,
        ))
    }
...
fn forced_contract_version() -> Option<u32> {
    std::env::var("UNIFFI_FORCE_CONTRACT_VERSION")
        .ok()
        .filter(|s| !s.is_empty())
