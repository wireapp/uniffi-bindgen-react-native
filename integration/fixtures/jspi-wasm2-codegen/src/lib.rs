...
// This fixture intentionally exercises wasm-bindgen's experimental JSPI API.
#[expect(deprecated)]
mod suspending_imports {
    use super::*;

    #[wasm_bindgen]
    extern "C" {
        #[wasm_bindgen(suspending, catch, js_namespace = globalThis, js_name = jspiFixtureRequest)]
        pub fn request(id: u32) -> Result<u32, JsValue>;
    }
}

#[cfg(target_arch = "wasm32")]
use suspending_imports::request;
...
// Granular JSPI must preserve the synchronous construction/subclassing surface.
...
impl MixedClient {
    ...
    #[uniffi::constructor]
    pub async fn open(id: u32) -> Result<Arc<Self>, FixtureError> {
        suspend(id)?;
        Ok(Self::new())
    }
    ...
    pub async fn refresh(&self, id: u32) -> Result<u32, FixtureError> {
        suspend(id)
    }

    pub async fn ordinary(&self) -> u32 {
        8
    }
}
...
#[uniffi::export]
impl SelectedConstructor {
    #[uniffi::constructor]
    pub fn new(id: u32) -> Result<Arc<Self>, FixtureError> {
        suspend(id)?;
        Ok(Arc::new(Self))
    }
