...
export async function run(bytes: Uint8Array) {
  ...
  try {
    ...
    // These declarations are checked by tsc against freshly generated bindings.
    class DerivedClient extends api.MixedClient {
      constructor() {
        super();
      }
    }
    ...
    check(
      value === 7 && client instanceof DerivedClient,
      "sync constructor remains public and subclassable",
    );
    const local: api.MixedClientLike = api.MixedClient.local();
    check(
      local.value() === 7,
      "unselected named constructor stays synchronous",
    );
    const refreshed: Promise<number> = client.refresh(901);
    await tick();
    settle(901, 42);
    check((await refreshed) === 42, "selected async method suspends");
    check(
      (await client.ordinary()) === 8,
      "unselected async method retains ordinary polling",
    );
    const opened: Promise<api.MixedClientLike> = api.MixedClient.open(902);
    await tick();
    settle(902, 0);
    const remote = await opened;
    check(remote.value() === 7, "selected named constructor suspends");
    const created: Promise<api.SelectedConstructorLike> =
      api.SelectedConstructor.create(903);
    await tick();
    settle(903, 0);
    ...
    check(
      selectedValue === 9,
      "selected primary constructor leaves methods synchronous",
    );
    if (
      !api.MixedClient.instanceOf(local) ||
      !api.MixedClient.instanceOf(remote) ||
      !api.SelectedConstructor.instanceOf(selected)
    ) {
      throw new Error("constructors must return concrete objects");
    }
    client.uniffiDestroy();
    local.uniffiDestroy();
    remote.uniffiDestroy();
    selected.uniffiDestroy();
    ...
    return "34 generated wasm2 JSPI cases passed";
  } finally {
