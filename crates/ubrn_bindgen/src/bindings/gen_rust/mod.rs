...
impl<'a> ComponentTemplate<'a> {
    ...
    fn ffi_function_named(&mut self, func: &FfiFunction, export_name: &str) -> TokenStream {
        ...
        let jspi = self.params.config.jspi_exports.contains(export_name) && !func.is_async();
        let annotation = if jspi {
        ...
        let function = if needs_call_status {
        ...
        if jspi {
            // wasm-bindgen emits its experimental API warning beside the function,
            // so scope the explicit JSPI opt-in to a module containing this export.
            let module = ident(&format!("{export_name}_jspi_export"));
            quote! {
                #[expect(deprecated)]
                mod #module {
                    use super::*;
                ...
                #[allow(unused_imports)]
                pub use #module::#foreign_func_ident;
            }
        } else {
            function
        }
