...
(async () => {
    ...
    async (t) => {
      for (const synchronous of [true, false]) {
        for (const notifyFirst of [true, false]) {
          ...
          // ES5 compilation does not preserve built-in Error subclass prototypes.
          t.assertTrue(caught instanceof Error);
          t.assertEqual((caught as Error).name, "JspiPollError");
  ...
  await asyncTest(
    ...
    async (t) => {
      ...
      t.assertTrue(error instanceof Error);
      t.assertEqual(error.name, "AbortError");
      t.assertEqual(freed, 1);
      t.assertEqual(consumed, 1);
      t.assertEqual(uniffiRustFutureHandleCount(), 0);
      t.end();
